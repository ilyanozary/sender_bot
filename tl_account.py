...
try:
    ...
    while i < 180:
        ...
        if step_login == 'success':
            ...
            # create a new pending mbot record so admin can immediately add another account
            try:
                new_uniq = utl.unique_id()
                cs.execute(f"INSERT INTO {utl.mbots} (cat_id,creator_user_id,api_id,api_hash,status,created_at,uniq_id) VALUES (1,{from_id},'{row_mbots['api_id']}','{row_mbots['api_hash']}',0,{int(time.time())},'{new_uniq}')")
                cs.execute(f"SELECT * FROM {utl.mbots} WHERE uniq_id='{new_uniq}'")
                new_row = cs.fetchone()
            except Exception:
                new_row = None

            if new_row is not None:
                cs.execute(f"UPDATE {utl.users} SET step='add_acc;{new_row['id']};type' WHERE user_id={from_id}")
                info_msg.reply_html(
                    text="✅ با موفقیت لاگین شد\n\n"+f"شماره: <code>{row_mbots['phone']}</code>\n\n"+"اکنون می توانید شماره جدیدی وارد کنید یا روش افزودن اکانت را انتخاب کنید:",
                    reply_markup={'resize_keyboard': True,'keyboard': [[{'text': 'زیپ'}, {'text': 'سشن'}, {'text': 'شماره'}],[{'text': utl.menu_var}]]}
                )
            else:
                # fallback to panel step if creating a new pending record failed
